// src/portals/company/hooks/useLeads.ts
// React Query hooks for Lead management (M13).
...
export function useAssignLead() {
  ...
  return useMutation({
    mutationFn: ({ leadId, fieldAgentId }: { leadId: string; fieldAgentId: string }) =>
      DsaService.assignLead(leadId, fieldAgentId),

    // Optimistic update — update list cache immediately before server confirms
    onMutate: async ({ leadId, fieldAgentId }) => {
      await queryClient.cancelQueries({ queryKey: leadKeys.all })

      // Snapshot current cache
      const snapshot = queryClient.getQueriesData<{ data: Lead[] }>({ queryKey: leadKeys.all })

      // Optimistically update any lead list in cache
      queryClient.setQueriesData<{ data: Lead[]; pagination: unknown }>(
        ...
        (old) => {
          ...
          return {
            ...
            data: old.data.map((l) =>
              l.id === leadId
                ? { ...l, status: 'ASSIGNED' as LeadStatus, fieldAgent: { id: fieldAgentId, fullName: 'Assigning…' } }
                : l
            ),
          }
        }
    ...
    onSuccess: (updatedLead) => {
      toast.success(`Lead assigned to ${updatedLead.fieldAgent?.fullName ?? 'agent'}.`)
...
export function useCreateLead() {
  ...
  return useMutation({
    ...
    onSuccess: (newLead) => {
      toast.success(`Lead created for ${newLead.portalUser?.fullName ?? 'new customer'}.`)
      queryClient.invalidateQueries({ queryKey: leadKeys.all })
    },
    onError: () => {
