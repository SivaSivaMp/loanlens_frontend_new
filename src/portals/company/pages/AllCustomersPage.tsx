    ...
    <div className="space-y-6 pb-12 relative">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Comprehensive tracker for every customer's loan application across all
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {apps.length === 0 ? (
                  ...
                  apps.map((app) => {
                    const initials = app.customer.fullName
                      .slice(0, 2)
                      .toUpperCase();
                    return (
                      <tr
                      ...
                      >
                        ...
                        <td className="py-4 px-4">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-bold border ${STATUS_COLORS[app.status] ?? "bg-slate-50 text-slate-600 border-slate-200"}`}
                          >
                            {app.status.charAt(0) +
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-900 text-sm">
                    {selectedApp.customer.fullName}
                  </p>
                  <p className="text-xs text-slate-500">
                    {selectedApp.product.productName}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 p-5 space-y-5">
              ...
              {!selectedApp.customer.isActive && (
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-3.5 text-xs text-rose-900 flex items-start gap-2.5">
                  ...
                  <div>
                    <span className="font-bold">Customer Inactive:</span>{" "}
                    Application is frozen. Reactivate the customer profile to
                    resume processing.
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    {
                      label: "Product",
                      value: selectedApp.product.productName,
                    },
                    {
                      label: "Loan Amount",
                      value: fmt(selectedApp.loanAmountRequested),
                    },
                    {
                      label: "EMI / Month",
                      value: `${fmt(selectedApp.emi)}/mo`,
                    },
                    { label: "Loan Type", value: selectedApp.loanType },
                    {
                      label: "Assigned Agent",
                      value: selectedApp.fieldAgent.fullName,
                    },
                  ].map((d) => (
                    <div
                      key={d.label}
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    value={drawerStatus}
                    onChange={(e) =>
                      setDrawerStatus(e.target.value as AppStatus)
                    }
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  {[
                    {
                      label: "Gross Bank Commission (est.)",
                      value: "\u20b924,000",
                    },
                    {
                      label: "Agent Earning (80%)",
                      value: "\u20b919,200",
                      highlight: true,
                    },
              ...
              <div className="pt-2 border-t border-slate-100">
                <button
                  onClick={() =>
                    setCustModal({
                      mode: selectedApp.customer.isActive
                        ? "deactivate"
                        : "reactivate",
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-sm w-full p-6 space-y-4 animate-in zoom-in-95 duration-150">
            <div className="flex items-start gap-3">
              <div
                className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${custModal.mode === "deactivate" ? "bg-rose-100 text-rose-600" : "bg-emerald-100 text-emerald-600"}`}
              >
