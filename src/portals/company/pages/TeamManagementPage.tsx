...
import {
  Search, UserPlus, Copy, Check, X,
  ChevronDown, Mail, AlertTriangle, Shield,
} from 'lucide-react'
import { useCompanyAgents, useInviteAgent, useDeactivateAgent, useReactivateAgent } from '../hooks/useCompany'
...
export const TeamManagementPage: React.FC = () => {
  ...
  const { data, isLoading, isError, refetch } = useCompanyAgents({ page, limit: 20 })
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Team Management</h1>
          <p className="text-sm text-slate-500 mt-1">Manage your field agent roster, track performance, and invite new members.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs">
          ...
          <p className="text-3xl font-bold text-slate-900">{pagination?.total ?? agents.length}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs">
        ...
        <div className="bg-white border border-indigo-200 rounded-xl p-5 shadow-xs">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Company Invite Code</p>
          <div className="flex items-center gap-2 mt-1">
            <span className="font-mono font-bold text-indigo-700 text-lg tracking-widest">FINS-7823</span>
            <button onClick={copyCode} className="p-1.5 rounded-md bg-indigo-50 hover:bg-indigo-100 text-indigo-600 transition-colors cursor-pointer">
              {codeCopied ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}
            </button>
          </div>
          <p className="text-[11px] text-slate-400 mt-1">Agents use this code during registration</p>
        </div>
      </div>
                    ...
                    >
                      <td className="py-4 px-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-semibold text-sm text-slate-900">{agent.fullName}</div>
                      ...
                      <td className="py-4 px-5 text-right">
                        <div className="inline-flex items-center gap-2">
                          <button
                            onClick={(e) => { e.stopPropagation(); navigate(`/company/team/${agent.id}`) }}
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); setToggleModal({ agent }) }}
                            disabled={deactivateAgent.isPending || reactivateAgent.isPending}
                          ...
                          >
                            {(deactivateAgent.isPending || reactivateAgent.isPending) ? 'Processing…' : agent.isActive ? 'Deactivate' : 'Reactivate'}
                          </button>
                        </div>
                      </td>
                    </tr>
          ...
          {pagination && pagination.totalPages > 1 && (
            <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 bg-slate-50/50">
              <p className="text-xs text-slate-500">
                Page {pagination.page} of {pagination.totalPages} &middot; {pagination.total} agents
              </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-md w-full p-6 animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-900">Invite Field Agent</h3>
            ...
            {inviteSent ? (
              ...
              <form onSubmit={handleSendInvite} className="space-y-4 mt-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-600 uppercase mb-1">Agent Full Name</label>
                  <input
                    type="text"
                    required
                ...
                <div className="flex items-center gap-3 p-3 bg-indigo-50/60 border border-indigo-100 rounded-xl">
                  ...
                  <p className="text-[11px] text-indigo-700">
                    Email includes company code <span className="font-mono font-bold">FINS-7823</span> + registration instructions.
                  </p>
                </div>
            ...
            <div className="flex items-start gap-3">
              <div
                className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${
                  toggleModal.agent.isActive ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'
                }`}
              >
                {toggleModal.agent.isActive ? <AlertTriangle className="w-5 h-5" /> : <Shield className="w-5 h-5" />}
              </div>
              <div>
                <h3 className="font-bold text-slate-900">
                  {toggleModal.agent.isActive ? 'Deactivate' : 'Reactivate'} {toggleModal.agent.fullName}?
                </h3>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-semibold text-white rounded-lg cursor-pointer ${
                  toggleModal.agent.isActive ? 'bg-rose-600 hover:bg-rose-700' : 'bg-emerald-600 hover:bg-emerald-700'
                }`}
              >
