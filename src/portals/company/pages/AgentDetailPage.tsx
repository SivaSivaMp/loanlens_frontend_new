...
const AppStatusBadge: React.FC<{ status: AppStatus }> = ({ status }) => {
  ...
  return (
    <span
      className={`px-2.5 py-1 text-xs font-semibold rounded-full ${cfg.cls}`}
    >
      {cfg.label}
    </span>
  );
...
const DonutChart: React.FC<{ agentPct: number }> = ({ agentPct }) => {
  ...
  return (
    <svg viewBox="0 0 100 100" className="w-32 h-32">
      ...
      <text x="50" y="60" textAnchor="middle" fontSize="7" fill="#6b7280">
        Agent
      </text>
    </svg>
  );
...
export default function AgentDetailPage() {
  ...
  const resolvedSplitPercent =
    splitPercent ?? agent?.splitRule?.agentPercent ?? 80;
  ...
  const handleSaveSplit = () => {
    updateSplitRule.mutate({
      ruleId: "default",
      agentPercent: resolvedSplitPercent,
    });
  };
    ...
    <div className="min-h-screen bg-[#fcf8ff] p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          ...
          <div
            className={`w-20 h-20 rounded-2xl flex items-center justify-center text-white font-bold text-2xl border-2 shrink-0 ${
              agent.isActive
                ...
                : "bg-slate-400 border-slate-200 opacity-60"
            }`}
          >
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
          ...
          <span className="ml-auto px-2 py-0.5 text-[10px] font-bold rounded-full bg-emerald-100 text-emerald-700">
            &#10003; Bank Verified
          </span>
        </div>
      </div>
        ...
        {[
          {
            ...
            value: agent.stats.searchesThisMonth,
            icon: Search,
            color: "indigo",
            isText: false,
          },
          {
            ...
            value: agent.stats.applicationsThisMonth,
            icon: TrendingUp,
            color: "sky",
            isText: false,
          },
          ...
          },
          {
            ...
            value: fmtINR(agent.stats.expectedCommissionThisMonth),
            icon: IndianRupee,
            color: "emerald",
            isText: true,
          },
          ...
          >
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center bg-${color}-100`}
            >
              <Icon className={`w-4 h-4 text-${color}-600`} />
            </div>
            <div>
              <p
                className={`${isText ? "text-xl" : "text-2xl"} font-bold text-slate-900`}
              >
                {value}
              </p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Last 5 loan applications handled by this agent
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        ...
        <p className="text-xs text-slate-500 mb-6">
          Adjust the agent's commission split using the slider below.
        </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-indigo-50 rounded-xl p-4 space-y-1">
                ...
                <p className="text-xl font-bold text-indigo-800">
                  {fmtINR(
                    Math.round(
                      (agent.commissionSummary.expectedTotal *
                        resolvedSplitPercent) /
                        100,
                    ),
                  )}
                </p>
                <p className="text-xs text-indigo-500">
                  {resolvedSplitPercent}% of pool
                </p>
              </div>
              <div className="bg-slate-50 rounded-xl p-4 space-y-1">
                ...
                <p className="text-xl font-bold text-slate-700">
                  {fmtINR(
                    Math.round(
                      (agent.commissionSummary.expectedTotal *
                        (100 - resolvedSplitPercent)) /
                        100,
                    ),
                  )}
                </p>
                <p className="text-xs text-slate-400">
                  {100 - resolvedSplitPercent}% of pool
                </p>
              </div>
            </div>
